  ...
  return (
    <div className={`upload-page ${darkMode ? "dark" : ""}`}>
      ...
      <div className="upload-header">
        ...
        <h2>AI-Based Zoonotic Disease Detection</h2>
        <p className="subtitle">
          Upload your RT-PCR report or symptom file for early disease analysis
        </p>
      </div>
        ...
        <div className="report-card">
          ...
          {ctRisk && (
            <p className={`risk-level-display ${getRiskClass(ctRisk.level)}`}>
              <strong>Risk Level (Ct-based):</strong> {ctRisk.level} (Min Ct:{" "}
              {ctRisk.text})
            </p>
          )}

          {/* --- AI Suggestions (only for known results) --- */}
          {report.disease?.toLowerCase() !== "unknown" &&
            report.result?.toLowerCase() !== "unknown" &&
