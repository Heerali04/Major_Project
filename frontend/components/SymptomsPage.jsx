...
export default function SymptomsPage({ darkMode = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Using test user_id if not found, as per existing logic
    ...
    try {
      ...
      // API call to the Flask backend endpoint
      const res = await axios.post("http://127.0.0.1:5000/analyze_symptoms", {
        user_id,
      ...
      console.error(err);
      setError(
        err.response?.data?.error || "Failed to get prediction. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  // Helper to determine the risk class for styling (classes are now assumed to be external)
      ...
      {report && (
        <div className="report-card" role="region" aria-label="Prediction Report">
          ...
          {report.suggestion?.["Risk Level"] && report.suggestion?.["Risk Probability"] && (
            <div className={`risk-level-display ${getRiskClass(report.suggestion["Risk Level"])}`}>
              <strong>Risk Level:</strong> {report.suggestion["Risk Level"]} (
              {(report.suggestion["Risk Probability"] * 100).toFixed(1)}%)
            </div>
          )}

          {/* Only show matched symptoms if any */}
          {report.matched_symptoms?.length > 0 && (
            <p>
              <strong>Matched Symptoms:</strong> {report.matched_symptoms.join(", ")}
            </p>
          )}
